...
%{
  This file may be \include'd in a score to produce the standard MIDI for most
  scores, so long as they follow certain conventions.

  Usage:

  midi-template.ly should be included at the *end* of the input file.  Before
  it are placed the required music by redefining specific variables, like this:
  ...
  #(define voice-postfixes
     ;; These names of the various voices
    '(("Soprano I" "SopranoOneMusic")
      ("Soprano II" "SopranoTwoMusic")
      ...))

  \include "bumby-hymnal-template.ly"

  Tempo is required, and should be all rests with inserted tempo markings.  All
  the other voice parts need to have no tempo markings.

  Metronome is optional and should contain the glock marking the beat.  Like all
  other voices it should be of the same length as all other voices.

  All of the voice parts must have exactly the same duration.  If they include 
