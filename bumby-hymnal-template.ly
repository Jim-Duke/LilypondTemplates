...
%{
  This file may be \include'd in a score to produce the standard products for
  all hymns in the South Bumby Supplementary Hymnal.

  It is intended to encapsulate all the standards for layout staves page
  layout standards, and MIDI output.  That way the files which include it can
  primarily focus on the music and lyrics.
